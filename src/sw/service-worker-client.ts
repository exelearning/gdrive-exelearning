...
function runtimeScope(): string {
  // APP_BASE_URL always ends with a slash (Vite normalises it).
...
async function registerRuntimeWorker(): Promise<RuntimeWorker> {
  ...
  // `runtimeBase` is the scope without the trailing slash, so
  // `buildRuntimeUrl()` can append `/<sessionId>/<entry>` directly.
 ...
 * Each entry is sliced into a fresh `ArrayBuffer` so the SW does not retain
 * references to the page's typed-array views.
 */
...
export async function unregisterSession(
  ...
  try {
    ...
    // Session cleanup failures must not surface — the SW drops the entry
    // when the tab closes anyway.
  }
}
