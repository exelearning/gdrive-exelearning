 ...
 * push it to Drive as a thumbnail; we never look inside `content.xml` or any
 * other editor-owned asset.
 *
 * Implements just enough of PKZIP to find an entry in the central directory
 * and decompress its payload via the browser's DecompressionStream. ZIP64,
 * encryption and multi-disk archives are not supported (eXeLearning does not
 * produce them).
 */
