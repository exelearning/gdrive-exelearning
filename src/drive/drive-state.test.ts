...
describe('parseDriveState', () => {
  ...
  it('rejects an open state without file ids', () => {
    expect(() => parseDriveState(JSON.stringify({ action: 'open', ids: [] }))).toThrow('at least one file id');
  });
});

describe('hasRemoteRevisionChanged', () => {
  it('compares Drive versions before modified time', () => {
    expect(hasRemoteRevisionChanged({ version: '1', modifiedTime: 'a' }, { version: '2', modifiedTime: 'a' })).toBe(true);
  });
});
